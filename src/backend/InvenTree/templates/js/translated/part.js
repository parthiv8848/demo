...
function loadPartTable(table, url, options={}) {
    ...
    // options.params['default_location_detail'] = true;
    // options.params['stock_location_detail'] = true;
    ...
    // Add MPN column
    columns.push({
        field: 'manufacturer_parts',
        title: '{% trans "MPN" %}',
        sortable: false,
        formatter: function (value, row) {
            console.log('Manufacturer Parts:', row.manufacturer_parts);
            if (row.manufacturer_parts && row.manufacturer_parts.length > 0) {
                return row.manufacturer_parts.map(mp => mp.MPN).join(', ');
            } else {
                return '{% trans "No MPNs available" %}';
            }
        }
    });
    ...
    // Add the default location column
    columns.push({
        field: 'stock_location_detail',
        title: '{% trans "Stock Locations" %}',
        sortable: true,
        formatter: function (value, row) {
            // Check if stock_location_detail exists and has elements
            if (row.stock_location_detail && row.stock_location_detail.length > 0) {
                // Map through the array and extract the names
                return row.stock_location_detail.map(loc => loc.name).join(', ');
            } else {
                // If no location details are available, return a placeholder
                return '<em>{% trans "No locations" %}</em>';
            }
        }
    });
    // columns.push({
    ...
    $(table).inventreeTable({
        ...
        showFooter: true,
        onPostBody: function() {
            console.log('Table data after rendering:', $(table).bootstrapTable('getData'));
