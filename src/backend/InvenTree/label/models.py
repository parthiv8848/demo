...
class PartLabel(LabelTemplate):
    ...
    def get_context_data(self, request):
        ...
        # Fetch related ManufacturerParts using Django ORM
        manufacturer_parts = part.manufacturer_parts.all()

        # Extract MPNs and descriptions from the queryset
        manufacturer_parts_data = [
            {'MPN': manufacturer_part.MPN}
            for manufacturer_part in manufacturer_parts
        ]

        # Function to remove text inside parentheses and brackets
        def clean_description(description):
            # Remove text inside parentheses and brackets
            return re.sub(r'\(.*?\)|\[.*?\]', '', description).strip()

        # Clean the part description
        cleaned_description = clean_description(part.description)

        # Construct the QR data URL
        qr_data_url = f"http://194.233.91.191:8000/part/{part.pk}/#"
        ...
        return {
            ...
            'manufacturer_parts': manufacturer_parts_data,
            'qr_data': qr_data_url,  # Updated to use the custom URL format
