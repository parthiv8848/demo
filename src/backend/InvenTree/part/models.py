...
class ManufacturerPart(models.Model):
    ...
    part = models.ForeignKey(
        ...
        related_name='manufacturer_parts_ok',  # Ensure this is unique across your models
    )
    
    manufacturer = models.ForeignKey(
        'company.Company',
        on_delete=models.CASCADE,
        related_name='manufactured_parts_ok',  # Ensure this is unique across your models
    )
    ...
    def __str__(self):
        return f"{self.manufacturer.name} | {self.MPN}"
...
def get_next_part_number():
    max_part = Part.objects.filter(name__regex=r'^V\d{6}$').aggregate(Max('name'))['name__max']
    if max_part:
        current_number = int(max_part[1:])
        next_number = current_number + 1
    else:
        next_number = 1
    return f'V{next_number:06}'
...
class Part(
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk and not self.name:
            # This is a new part being created without a name
            # Get the highest existing part number
            max_part = Part.objects.filter(name__regex=r'^V\d{6}$').aggregate(Max('name'))['name__max']
            if max_part:
                ...
                # If no existing parts, start from 1
                next_number = 1

            # Format the new part number
            self.name = f'V{next_number:06}'
        _new = False
        if self.pk:
            try:
    ...
    def generate_ipn(self):
        if self.category and self.category.pathstring:
            prefix = self.category.pathstring.split('/')[0].upper()[:3]
        else:
            prefix = 'UNC'  # For uncategorized parts

        # Find the last IPN with this prefix
        last_ipn = Part.objects.filter(IPN__startswith=f"{prefix}_").order_by('-IPN').first()

        if last_ipn:
            # Extract the number part and increment it
            last_number = int(last_ipn.IPN.split('_')[-1])
            new_number = last_number + 1
        else:
            new_number = 1

        # Generate a new unique IPN
        while True:
            new_ipn = f"{prefix}_{str(new_number).zfill(6)}"
            if not Part.objects.filter(IPN=new_ipn).exists():
                return new_ipn
            new_number += 1

    def save(self, *args, **kwargs):
        if not self.IPN:
            self.IPN = self.generate_ipn()
        super().save(*args, **kwargs)

  

    name = models.CharField(
        max_length=100, default=get_next_part_number, help_text=_('Part number'), verbose_name=_('Number')
    )
...
def set_part_ipn(sender, instance, **kwargs):
        if not instance.IPN:
            instance.IPN = instance.generate_ipn()
