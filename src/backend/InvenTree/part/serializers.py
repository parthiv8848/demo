...
from rest_framework import serializers
from .models import Part, ManufacturerPart
from stock.models import StockLocation,StockItem


class StockLocationSerializer(serializers.ModelSerializer):
    """Serializer for StockLocation details."""
    class Meta:
        model = StockLocation
        fields = ['pk', 'name']

# class StockItemLocationSerializer(serializers.ModelSerializer):
#     """Serializer for extracting location details from StockItem."""
...
class PartSerializer(
    ...
    """
    manufacturer_parts = ManufacturerPartSerializer(many=True, read_only=True)
    # default_location_detail = StockLocationSerializer(source='default_location', read_only=True)
    stock_location_detail = serializers.SerializerMethodField()
    ...
    def get_stock_location_detail(self, obj):
        """Return a list of unique stock location details."""
        # Assuming `stock_items` is a related name on the StockItem model pointing back to Part
        unique_locations = {item.location for item in obj.stock_items.all() if item.location}
        return StockLocationSerializer(unique_locations, many=True).data
    def __init__(self, *args, **kwargs):
